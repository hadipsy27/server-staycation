...
module.exports = {
  ...
  deleteFeature: async (req, res) => {
    ...
    try {
      ...
      for(let i = 0; i < feature.length; i++){
        if(item.feature[i]._id.toString() === feature._id.toString()){
          item.feature.pull({_id : feature._id})
          await item.save()
        }
      }
      await fs.unlink(path.join(`public/${feature.imageUrl}`))
  ...
  deleteActivity: async (req, res) => {
    ...
    try {
      ...
      for(let i = 0; i < activity.length; i++){
        if(item.activity[i]._id.toString() === activity._id.toString()){
          item.activity.pull({ _id : activity._id })
          await item.save()
        }
      }
      await fs.unlink(path.join(`public/${activity.imageUrl}`))
  ...
  actionReject: async (req, res) => {
    ...
    try {
      ...
      await booking.save()
      // console.log(booking.payments.status)
